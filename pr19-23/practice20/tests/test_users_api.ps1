# Practice 20 - MongoDB User API Tests
param(
    [string]$baseUrl = "http://localhost:3089",
    [switch]$verbose
)
...
Write-TestHeader "Practice 20 - MongoDB User API Tests"

try {
    # Test 1: Health Check
    Write-Host "Testing health endpoint..." -ForegroundColor Yellow
    $healthResponse = Invoke-RestMethod -Uri "$baseUrl/health" -Method Get -ErrorAction Stop
    Write-TestResult "Health Check" ($healthResponse.status -eq "healthy") $healthResponse
}
catch {
...
$newUser = @{
    first_name = "John_$(Get-Random)"
    last_name = "Doe_$(Get-Random)"
    age      = $(Get-Random) % 30
} | ConvertTo-Json
...
try {
    ...
    Write-TestResult "Create User" ($null -eq $createResponse._id) $createResponse
...
try {
    ...
    Write-TestResult "Update User" ($updateResponse.age -eq 31) $updateResponse
    $updateResponse
} catch {
